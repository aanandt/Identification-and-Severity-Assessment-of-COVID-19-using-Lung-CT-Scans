function [] = preprocessing()
	...
	inputDir = '/cbr/anand/ResearchWork/Covid_2021/Dataset/Test/FInalTest/'
	%'/speech/tmp/anand/Covid_Dataset/MY_TestData/COVID19_Mosmed/InputData/'
	%'/speech/tmp/anand/Covid_Dataset/MY_TestData/COVID_LDCT/Dataset-S1_LDCT/'
	%'/cbr/anand/ResearchWork/Covid_2021/Dataset/Test/FInalTest/' 
	outputDir = 'Preprocessed_Data_AbelationStudy_Updated/SPGC_Test/'
	%input_file = '../Dataset/Slice_level_labels.csv';
	Mask_path = 'TempDir/Matlab_Mask_files_Test_SPGC/';
	...
	%{
	patients = Read_folders(inputDir);
	for i = 1 : 1 : numel(patients)
		patient = patients(i).name;
		folder_path = fullfile(inputDir,patient)
...
function [Severity_score_patient] = Extract_Dicom_Preprocessing(folder_path, outputDir, Mask_path)
	...
	parfor pat_index = 1: 1 : numel(patients)
		...
		for slice_index = 1 : 1: len
			...
			%SaveImage(org_seg_Image, outputDir, 'GMM_filter_img_with_imfill', patients_name, slice_index)
			...
			if ((~ all(seg_Image(:) < -1000) & flag == 1) & ~(slice_index < start_) & ~(slice_index > end_))%(numel(dicomfiles) - 5)
			    ...
			    try
			        ...
			        %GMModel = fitgmdist(S,3,'CovarianceType','diagonal','Options',options);
				...
				GMM_fill_morpho_seg = final_mask .* seg_Image;
				GMM_fill_morpho_org_seg = final_mask .* org_seg_Image;

				%x = imgHyperb2D(org_seg_Image);

				SaveImage(GMM_fill_morpho_org_seg, outputDir, 'GMM_Noimfill_NoBoundary', patients_name, slice_index)
				...
				SaveImage(GMM_fill_morpho_org_seg, outputDir, 'GMM_imfill_NoBoundary', patients_name, slice_index)
				...
				SaveImage(my_img, outputDir, 'GMM_Noimfill_Boundary', patients_name, slice_index)
				...
				SaveImage(my_img, outputDir, 'GMM_imfill_Boundary', patients_name, slice_index)
			    ...
			    SaveImage(my_img, outputDir, 'my_img', patients_name, slice_index);

				SaveImage(final_processed_img, outputDir, 'Final_preprocessed_img', patients_name, slice_index);
				
				SaveImage(segmented_img, outputDir, 'Final_preprocessed_img_without_border', patients_name, slice_index);
...
function [] = SaveImage(my_img, outputDir, category, patients_name, slice_index)
    ...
    dest_dir = fullfile(outputDir, category , patients_name);
