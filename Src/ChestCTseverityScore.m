function [] = ChestCTSeverityScore(input_path, Mask_path, CTSS_GT_path)
	GT_CTSS_score = Get_CTSS_score(CTSS_GT_path)
	...
	for pat_index = 1 : 1 : numel(patients)
		patients_name = patients(pat_index).name
			...
			masks = load(fullfile(Mask_path, patients_name, 'mask.mat')).data();
			CTscans = load(fullfile(Mask_path, patients_name, 'image.mat')).data();
			...
			for slice_index = 1: 1 : len
				...
				if ((lung_involvement >= 6) & (~(slice_index < start_) & ~(slice_index > end_)))
			        ...
			        PreprocessedImage = Preprocessing(temp, Image, len, slice_index, patients_name);
			...
			%GT_CTSS_score{pat_index,2}
			patientswise_CTSS{pat_index,1} = str2num(GT_CTSS_score{pat_index,2});
			patientswise_CTSS{pat_index,2} = round(mean(Severity_Score(:,1))) + round(mean(Severity_Score(:,2))) + round(mean(Severity_Score(:,3))) + round(mean(Severity_Score(:,4))) + round(mean(Severity_Score(:,5)))
			patientswise_CTSS{pat_index,3} = round(max(Severity_Score(:,1))) + round(max(Severity_Score(:,2))) + round(max(Severity_Score(:,3))) + round(max(Severity_Score(:,4))) + round(max(Severity_Score(:,5)))
			%patientswise_CTSS{pat_index,4} = round(mean(Severity_Score(:,1))) + round(mean(Severity_Score(:,2))) + round(mean(Severity_Score(:,3))) + round(mean(Severity_Score(:,4))) + round(mean(Severity_Score(:,5)))
			%patientswise_CTSS{pat_index,5} = round(mean(Severity_Score(:,4)))
			%patientswise_CTSS{pat_index,6} = round(mean(Severity_Score(:,5)))
			%patientswise_CTSS{pat_index,2} = round(mean(Severity_Score))

	end
	x = cell2mat(patientswise_CTSS(:,1));
	y = cell2mat(patientswise_CTSS(:,2));
	corrcoef(x,y)
	writecell(patientswise_CTSS,'CTSS_results.csv')
end

function [all_patient_label] = Get_CTSS_score(input_file)
	...
	while ~feof(fileID)	
	    ...
	    all_patient_label{index,2} = sdata{2};
	    %all_patient_label{index,3} = patient_label(3);
	    %all_patient_label{index,4} = patient_label(5);
	    %all_patient_label{index,5} = patient_label(7);
	    index = index + 1;
	    %data = fgetl(fileID);
	end
end
function[Infection_mask] =  Preprocessing(curr_mask, Image, len, slice_index, patients_name)
...
%     RLL=[];
    ...
    severity(1) = Find_LobeScore(LLL);
    severity(2) = Find_LobeScore(LUL);
    severity(3) = Find_LobeScore(RLL);
    severity(4) = Find_LobeScore(RUL);
    severity(5) = Find_LobeScore(RML);
