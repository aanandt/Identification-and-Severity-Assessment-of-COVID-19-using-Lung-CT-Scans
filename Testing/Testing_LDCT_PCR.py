...
df.to_csv('ClassificationResults/Test_result_LDCT_PCR'+feature_extractor+'.csv')
...
if not os.path.exists('ClassificationResults'):
    os.makedirs('ClassificationResults')
plt.show()
plt.savefig('ClassificationResults/confusionmatrix_'+ 'LDCT_PCR'+feature_extractor+'.png')

skplt.metrics.plot_roc_curve(gt_label.argmax(axis=1), pred_label)
plt.show()
plt.savefig('ClassificationResults/roc_curve_LDCT_PCR'+feature_extractor+ '.png')
