...
def apply(image, directions, model=None, force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
    if model is None:
    ...
    inimg_raw = np.reshape(inimg_raw, (1,512,512))
    # for tiff images
    '''
    inimg_raw = sitk.GetArrayFromImage(image)
    inimg_raw = inimg_raw.astype(np.int16) - 1024
    inimg_raw = np.reshape(inimg_raw, (1,512,512))
    directions = np.asarray(image.GetDirection())
    '''
    #for nifti images
    #inimg_raw = sitk.GetArrayFromImage(image)
    #inimg_raw = np.reshape(image, (1,512,512))
...
def apply_fused(image, directions, basemodel = 'LTRCLobes', fillmodel = 'R231', force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
    ...
    res_l = apply(image, directions, mdl_l, force_cpu=force_cpu, batch_size=batch_size,  volume_postprocessing=volume_postprocessing, noHU=noHU)
    logging.info("Apply: %s" % fillmodel)
    res_r = apply(image, directions, mdl_r, force_cpu=force_cpu, batch_size=batch_size,  volume_postprocessing=volume_postprocessing, noHU=noHU)
